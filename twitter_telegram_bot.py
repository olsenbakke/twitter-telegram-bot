...
def mark_as_sent(chat_id: Any, tweet_id: str) -> None:
    ...
    # نوشتن روی هارد دیسک از این بخش حذف شد تا Event Loop مسدود نشود. ذخیره‌سازی در هدر بکاپ دوره‌ای انجام می‌شود.
...
def build_tweet_message(username: str, title: str, fa_text: Optional[str], is_alert: bool, image_url: Optional[str] = None) -> str:
    ...
    # تزریق شیک تصویر هیدن برای دور زدن محدودیت کاراکتر کپشن تلگرام
    ...
    # اصلاح باگ تگ متون جمع‌شونده تلگرام
...
async def send_tweet_entry(chat_id: Any, username: str, entry: Any, bot: Any) -> Tuple[bool, str]:
    ...
    # ساخت پیام یکپارچه همراه با پیش‌نمایش تصویر تعبیه شده
    ...
    try:
        # همیشه ارسال متن یکپارچه؛ مدیا به صورت خودکار از لینک پنهان لود خواهد شد
...
async def auto_backup(app: Application) -> None:
    ...
    while True:
        try:
            # دیتای حافظه موقت RAM ابتدا روی دیسک ذخیره می‌شود تا مانع I/O Blocking شود
                ...
                for path, caption in caption_map.items():
                    if os.path.exists(path):
                        try:
                            ...
                            logger.warning(f"Auto-backup {path} failed: {e}")
                logger.info(f"Auto-backup sent to admin {ADMIN_CHAT_ID}")
        except Exception as e:
            logger.error(f"Auto-backup error: {e}")
        await asyncio.sleep(min(BACKUP_INTERVAL, 300)) # حداقل هر ۵ دقیقه یک بار دیتابیس لوکل فلاش می‌شود
        ...
        for username, info in list(tracked.items()):
            try:
                ...
                if last_id and not found_last_id:
                    ...
                    # اصلاح باگ ترتیب معکوس به طوری که جدیدترین‌ها همواره حفظ شوند
                ...
                # پیمایش از قدیم به جدید برای حفظ زنجیره زمانی توییت‌ها
...
async def cmd_export(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    _flush_dedup_to_file()  # فلاش کردن دیتا پیش از گرفتن فایل بکاپ نهایی
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if filename not in ("tracked_users.json", "filters.json", "sent_ids.json"):
        await update.message.reply_text("فقط فایل‌های json مجاز سیستم قبول می‌شوند.")
        return
    try:
